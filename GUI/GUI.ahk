...
gui_autoexecute:
    ...
    ; Busca o caminho do notepad++.exe
    GoSub, NotepadPath

    ; Define o local padrão de instalação do AutoSystem
    aspath := "C:\autosystem\" 
...
; LAUNCH GUI
;-------------------------------------------------------------------------------
^CapsLock:: ; launching this from the other AHK script which catches everything related to win key
gui_spawn:
    if gui_state != closed
    {
    ...
    Gui, Add, Text, %gui_control_options% vgui_main_title, Digite o comando desejado: (?)
...
NotepadPath:
If FileExist("C:\Notepad++ Portable\notepad++.exe") {        ; Se existe o caminho,
		st := "C:\Notepad++ Portable\notepad++.exe" ; A variavel recebe ele
        SplashTextOn,225,25,Buscador Notepad++, Notepad++ Portable encontrado ; Abre uma tela de aviso rápido informando qual foi encontrado
        Sleep 1000
        SplashTextOff

        }
    else {
        st := "notepad++.exe" ; A variavel recebe ele
        SplashTextOn,225,25,Buscador Notepad++, Notepad++ (Instalado) encontrado ; Abre uma tela de aviso rápido informando qual foi encontrado
        Sleep 1000
        SplashTextOff
        }
...
; %gui_control_options% = gui_control_options := "xm w220 " . cForeground . " -E0x200" - removes border around Edit controls

gui_register_case_open_label:
    Gui, Add, Text, %gui_control_options% %cRed%, Nome:
    Gui, Add, Edit, %gui_control_options% %cRed%
	Gui, Add, Text, %gui_control_options% %cRed%, CNPJ:
	Gui, Add, Edit, %gui_control_options% %cRed%
	Gui, Add, Text, %gui_control_options% %cRed%, Telefone:
	Gui, Add, Edit, %gui_control_options% %cRed%
	Gui, Add, Text, %gui_control_options% %cRed%, Email:
	Gui, Add, Edit, %gui_control_options% %cRed%
...
gui_register_case() {
    ...
    {
        gui_state = search
        Gosub, gui_register_case_open_label
		
    }
}

gui_register_new_case() {
	if gui_state != search
    {
        gui_state = search
        Gosub, gui_register_case_open_label
...
gui_search_add_elements:
    Gui, Add, Text, %gui_control_options% %cAqua%, %gui_search_title%
    Gui, Add, Edit, %gui_control_options% %cAqua% vgui_SearchEdit -WantReturn
    Gui, Add, Button, x-10 y-10 w1 h1 +default ggui_SearchEnter ; hidden button
    GuiControl, Disable, Command
    Gui, Show, AutoSize
    return
            ...
            If A_LoopReadLine contains if
            {
                IfInString, A_LoopReadLine, Command
                    IfInString, A_LoopReadLine, =
                    {
                        ...
                        StringReplace, trimmed, trimmed, `%A_Space`%,{espaço}, All
                        tooltiptext .= trimmed
                        tooltiptext .= "`n"
